	...
	<div class="p-portlet-body">
		...
		<form action="" method="post" class="p-form-table">
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">
				...
				<input type="text" name="villetitle" id="villetitle" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("villetitle") ?: $ville->city_title; ?>" placeholder="Entrez le nom" autofocus required />
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[2]) != 0 ? ' has-error' : '';  ?>">
				...
				<select name="zid" class="form-control p-form-ctrl p-form-ctrl-sm">
					...
					<?php if($ville->zid === $item->id): ?>
					
					<option value="<?= $item->id ?>" selected><?= $item->zone_title; ?></option>	

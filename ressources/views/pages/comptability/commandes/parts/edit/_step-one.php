...
<div class="p-portlet-wizard-form">
	...
	<form action="" method="post" class="p-form-table">
		...
		<div class="form-group mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">
			...
			<input type="text" name="dating" id="dating" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("dating") ?: $commande->dating; ?>" placeholder="Ex., 01/01/2000 - 31/01/2000" autofocus required />
		...
		<div class="form-group mb-3<?= !empty($error) && count($error[2]) != 0 ? ' has-error' : '';  ?>">
			...
			<textarea name="description" class="form-control p-form-ctrl p-form-ctrl-sm" placeholder="Entrez le descriptif de la commande" required><?= get_input("description") ?: $commande->description; ?></textarea>
		...
		<button type="submit" name="steponesubmit" class="btn btn-sm btn-primary p-btn-primary mt-4 mb-3">Suivant</button>

...
<div class="p-portlet-wizard-form">
	...
	<h3 class="h5 pt-4 pb-4 text-muted">Taxe Municipale</h3>
	...
	<form action="" method="post" class="p-form-table">
		...
		<div class="form-row">
		    ...
		    <div class="form-group col-md-6 mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">
				...
				<select name="odp" class="form-control p-form-ctrl p-form-ctrl-sm">

	                <option value="Oui" <?= $proforma->odp === "Oui" ? "selected" : ""; ?>>Oui</option>

	                <option value="Non" <?= $proforma->odp === "Non" ? "selected" : ""; ?>>Non</option>

				</select>

			<?= !empty($error[1]) 
				? '<span class="help-block mt-1">' . $error[1] . '</span>' 
				: '<small class="text-muted">Confimez ou infirmez la taxe relative à l\'Occupation du Domaine Publique.</small>';
			?>

		    </div>

		    <?php if($proforma->alcool_type === "Non"): ?>

		    <div class="form-group col-md-6 mb-3<?= !empty($error) && count($error[2]) != 0 ? ' has-error' : '';  ?>">

		     	<label for="tm">TM</label>

				<select name="tm" class="form-control p-form-ctrl p-form-ctrl-sm">

	                <option value="Oui" <?= $proforma->tm === "Oui" ? "selected" : ""; ?>>Oui</option>

	                <option value="Non" <?= $proforma->tm === "Non" ? "selected" : ""; ?>>Non</option>

				</select>

			<?= !empty($error[2]) 
				? '<span class="help-block mt-1">' . $error[2] . '</span>' 
				: '<small class="text-muted">Confimez ou infirmez la taxe relative à la taxe Municipale.</small>';
			?>
		...
		<a href="<?= WURI . '?r=' . $m[1] . '/edition/etape-5/' . $ID . '/'; ?>" class="btn btn-sm btn-primary p-btn-primary-outline mt-4 mb-3">Précédent</a>

		<button type="submit" name="stepsixsubmit" class="btn btn-sm btn-primary p-btn-primary mt-4 mb-3 float-right">Suivant</button>

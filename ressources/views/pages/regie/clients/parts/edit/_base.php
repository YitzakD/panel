	...
	<div class="p-portlet-body">
		...
		<form action="" method="post" class="p-form-table">
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">
				...
				<input type="text" name="entname" id="entname" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("entname") ?: $client->e_name; ?>" placeholder="Entrez le nom" autofocus required />
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[2]) != 0 ? ' has-error' : '';  ?>">
				...
				<input type="text" name="interlocuteur" id="interlocuteur" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("interlocuteur") ?: $client->c_name; ?>" placeholder="Entrez le nom de l'interlocuteur" required />
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[3]) != 0 ? ' has-error' : '';  ?>">
				...
				<input type="email" name="entmail" id="entmail" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("entmail") ?: $client->a_mail; ?>" placeholder="Entrez l'adresse e-mail de l'interlocuteur" required />
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[4]) != 0 ? ' has-error' : '';  ?>">
				...
				<input type="number" name="entphone" id="entphone" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("entphone") ?: $client->contacts; ?>" placeholder="Entrez le numéro de téléphonique." required />
			...
			<div class="form-group mb-3<?= !empty($error) && count($error[5]) != 0 ? ' has-error' : '';  ?>">
				...
				<select name="type" class="form-control p-form-ctrl p-form-ctrl-sm">
                    ...
                    <option value="RP" <?= $client->type === "RP" ? "selected" : ""; ?>>Régie Publicitaire</option>

                    <option value="AC" <?= $client->type === "AC" ? "selected" : ""; ?>>Agence en Communication</option>

                    <option value="IP" <?= $client->type === "IP" ? "selected" : ""; ?>>Institution Publique</option>

                    <option value="II" <?= $client->type === "II" ? "selected" : ""; ?>>Institution Internationale</option>

                    <option value="AP" <?= $client->type === "AP" ? "selected" : ""; ?>>Autre / Personne</option>

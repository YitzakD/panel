...
<div class="p-portlet-wizard-form">
	...
	<h3 class="h5 pt-4 pb-4 text-muted">Choix de panneaux</h3>
	    ...
	    <div class="form-group mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">
			...
			<?php foreach($zones as $zone): ?>
				...
				<?php
					...
					$allsboards = find_signboards($zone->id, $commune->id, " AND (signboards.size='$fid') AND (signboards.etat='$state')");

				?>

					<?php foreach($allsboards as $item): ?>

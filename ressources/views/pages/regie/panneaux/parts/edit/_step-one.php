...
<div class="p-portlet-wizard-form">
	...
	<form action="" method="post" class="p-form-table">
		...
		<div class="form-row">
			...
			<div class="form-group col-md-8 mb-3<?= !empty($error) && count($error[1]) != 0 ? ' has-error' : '';  ?>">

				<label for="nbr" class="mb-0">Identifiant&nbsp;<i class="text-danger font-weight-bold align-top">*</i></label>

				<input type="number" name="nbr" id="nbr" class="form-control p-form-ctrl p-form-ctrl-sm" value="<?= get_input("nbr") ?: $signboard->nbr; ?>" placeholder="Entrez l'identifiant" autofocus required />
			...
			<div class="form-group col-md-4 mb-3<?= !empty($error) && count($error[3]) != 0 ? ' has-error' : '';  ?>">
				...
				<label for="etat" class="mb-0">Etat</label>

				<select name="etat" class="form-control p-form-ctrl p-form-ctrl-sm">


	                <option value="ras" <?= $signboard->etat === "ras" ? "selected" : ""; ?>>RAS</option>

	                <option value="en reparation" <?= $signboard->etat === "en reparation" ? "selected" : ""; ?>>En réparation</option>

	                <option value="accidente" <?= $signboard->etat === "accidente" ? "selected" : ""; ?>>Accidenté</option>

	                <option value="coupe" <?= $signboard->etat === "coupe" ? "selected" : ""; ?>>Coupé</option>

				</select>	

			<?= !empty($error[3]) 
				? '<span class="help-block mt-1">' . $error[3] . '</span>' 
				: '<small class="text-muted">Confirmez l\'état du panneau dans la liste ci-dessus.</small>'; 
			?>
		...
		<div class="form-row">
			...
			<div class="form-group col-md-7 mb-3<?= !empty($error) && count($error[3]) != 0 ? ' has-error' : '';  ?>">
				...
				<select name="cid" id="cid" class="form-control p-form-ctrl p-form-ctrl-sm">
					...
					<?php if($signboard->cmune === $item->id): ?>
					
					<option value="<?= $item->id ?>" selected><?= $item->cmune_title; ?></option>	

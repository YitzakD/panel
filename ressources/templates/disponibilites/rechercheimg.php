...
if($zid == "0") {
	...
	$zonename = "Tout le térritoie";

} else {

	$fzone = find_one("zones", "id", $zid);

	$zonename = $fzone->zone_title;

}

$state = "ras";

$sizefinder = find_one("sizes", "id", $fid);

$readeable_indate =  date('j.m.y', strtotime($debut));

$readeable_outdate =  date('j.m.y', strtotime($fin));

$filename = "Disponibilié-en-image-du-$readeable_indate-au-$readeable_outdate.pdf";
...
<page backtop="5mm" backleft="10mm" backright="10mm" backbottom="5mm" footer="page;">
		...
		<tr>
			...
			<td class="box-75 text-uppercase">
                ...
                <h3 class="document-title">
                    ...
                    <small class="text-grey"><?= "Du " . date('j/n/Y', strtotime($debut)) . " au " . date('j/n/Y', strtotime($fin)); ?></small>
    ...
    <table id="mytable">
        ...
        <tbody>
         ...
         <tr>
            ...
            <td class="box-33 text-uppercase ts-zx text-grey">Zone</td>

            <td class="box-33 text-uppercase ts-zx text-grey text-center">Format</td>

            <td class="box-33 text-uppercase ts-zx text-grey text-right">Type</td>

        </tr>

        <tr>

            <td class="box-33 text-uppercase"><?= $zonename ?></td>

            <td class="box-33 text-center"><?= $sizefinder->size . "m²" ?></td>

            <td class="box-33 text-right">Image</td>
	...
	<?php if($zid == 0): ?>
		...
		<?php $communescounter = cell_count("cmunes", "zid", $zone->id); ?>
        ...
        <tbody><tr>
            ...
            <td class="box-10" scope="col">#</td>

            <td class="box-10" scope="col">Format</td>

            <td class="box-80" scope="col">Situation géographique</td>
        ...
        <?php if($communescounter > 0): ?>
            ...
            <?php foreach($communes as $commune): ?>
            	...
            	<?php $fsbcounter = find_fsignboards_nbr($zone->id, $commune->id, $fid, $state); ?>
                    
                <?php $bfsbounter = find_signboards_tbf_nbr($debut, $fin, $zone->id, $commune->id, $fid, $state); ?>

                <?php $tdsbcounter = $fsbcounter + $bfsbounter; ?>

                <?php $fsboards = find_fsignboards($zone->id, $commune->id, $fid, $state); ?>

                <?php $bfsboards = find_signboards_tbf($debut, $fin, $zone->id, $commune->id, $fid, $state); ?>

                <?php $freesboards = array_merge($fsboards, $bfsboards); ?>

                <?php if($tdsbcounter > 0): ?>    

                <table class="table-stripped mb-5">
                    
                <thead><tr>
                    
                    <th class="text-uppercase bolder text-green">

                        <?= $commune->cmune_title; ?> (<?= $tdsbcounter; ?>)
                ...
                <img src="<?= $item->file_road; ?>" style="width:100%;" />
                
                <table class="table-stripped">
                    
                    <tbody><tr>

                        <td class="box-10 bg-green"><?= "DEV-" . $item->nbr ?></td>
                        
                        <td class="box-10 bg-green"><?= $item->size . "m²"; ?></td>
                        
                        <td class="box-80 bg-green"><?= $item->geoloc; ?></td>
	...
	<?php else: ?>
		...
		<?php $zone = find_one("zones", "id", $zid); ?>

...
$statementcounter = cell_count("init_payement", "client_id", $client->id);

$statement = find_all("init_payement", "client_id", $client->id);

$filename = "Liste-des-transactions-DEV-CLIENT-($client->e_name).pdf";
...
<style type="text/css">
    ...
    table td.box-23-25 {width:23.25%;}
...
<page backtop="5mm" backleft="10mm" backright="10mm" backbottom="5mm">
		...
		<tr>
			...
			<td class="box-75 text-uppercase">
                ...
                <h3 class="document-title">
                    ...
                    <small class="text-grey">Liste des transactions</small>
	...
	<table class="table-stripped">
		...
		<tr>
            ...
            <th scope="col">Acomptes Perçus</th>
        ...
        <?php foreach($statement as $item): ?>
			...
			<?php $acture = find_one("bills", "pf_id", $item->pf_id); ?>
			...
			<tr>
				...
				<td class="box-23-25 text-uppercase bg-striped"><?= '309F-' . date('jny',strtotime($item->created)) . '/' . $item->pf_id; ?></td>

                <td class="box-23-25 bg-striped"><?= number_format($item->ttc, 0, '', '.'); ?></td>

                <td class="box-23-25 text-capitalize bg-striped"><?= number_format($item->tac, 0, '', '.'); ?></td>

                <td class="box-23-25 text-capitalize bg-striped"><?= number_format($item->rap, 0, '', '.'); ?></td>
			...
			<tr>
				...
				<td class="box-23-25 text-capitalize borderbottom bordertop"><?= '309F-' . date('jny',strtotime($item->created)) . '/' . $item->pf_id; ?></td>

                <td class="box-23-25 borderbottom bordertop"><?= number_format($item->ttc, 0, '', '.'); ?> </td>

                <td class="box-23-25 text-capitalize borderbottom bordertop"><?= number_format($item->tac, 0, '', '.'); ?></td>

                <td class="box-23-25 text-capitalize borderbottom bordertop"><?= number_format($item->rap, 0, '', '.'); ?></td>
		...
		<tr>
			...
			<td colspan="4" class="empty-table text-center">
				...
				<span class="text-grey">Vous n'avez aucune transactions pour le moment</span>

...
$banquescounter = count_all("init_bank");

$banque= find_all("init_bank", " ORDER BY id DESC");

$filename = "Liste-des-rapprochements-bancaire.pdf";
...
<page backtop="5mm" backleft="10mm" backright="10mm" backbottom="5mm">
	...
	<table id="mytable" class="mb-6">
		...
		<tr>
			...
			<td class="box-75 text-uppercase"><h3 class="document-title">Liste<br>rapprochements</h3></td>
	...
	<table class="table-stripped">
		...
		<?php if($banquescounter > 0): ?>

        <?php $i = 1; ?>
		
        <?php foreach($banque as $item): ?>

			<?php $z = $i++ ?>

			 <?php $pointbanque = find_one("point_bank", "init_id", $item->id); ?>

			<?php if($z & 1): ?>

			<tr>

				<td class="box-25 text-capitalize bg-striped">

                    <div class="text-uppercase"><?= month_convert($item->month); ?></div>
                    <small class="text-grey text-uppercase"><?= $item->year; ?></small>

                </td>

                <td class="box-25 bg-striped"><?= number_format($pointbanque->credit, 0, '', '.'); ?></td>

                <td class="box-25 text-capitalize bg-striped"><?= number_format($pointbanque->debit, 0, '', '.'); ?></td>
        
                <td class="box-25 bg-striped"><?= number_format($pointbanque->solde, 0, '', '.'); ?></td>
			...
			<tr>
				...
				<td class="box-25 borderbottom bordertop">
				...
				<td class="box-25 text-capitalize borderbottom bordertop"><?= number_format($pointbanque->credit, 0, '', '.'); ?></td>

                <td class="box-25 borderbottom bordertop"><?= number_format($pointbanque->debit, 0, '', '.'); ?></td>

                <td class="box-25 text-capitalize borderbottom bordertop"><?= number_format($pointbanque->solde, 0, '', '.'); ?></td>
		...
		<tr>
			...
			<td colspan="4" class="empty-table text-center">
				...
				<span class="text-grey">Vous n'avez aucun rapprochement pour le moment</span>

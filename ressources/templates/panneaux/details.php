...
function lastcamps($nbr) {

    global $db;

    $q = $db->prepare("SELECT rsv.*, init_rsv.id, init_rsv.camp_name, init_rsv.debut, init_rsv.fin, init_rsv.client_id, init_rsv.etat, clients.e_name FROM rsv INNER JOIN init_rsv ON init_rsv.id = rsv.init_id LEFT JOIN clients ON clients.id = init_rsv.client_id WHERE rsv.nbr = ? AND init_rsv.etat != 'En attente' ORDER BY init_rsv.id DESC");
...
$lastcamps = lastcamps($signboard->nbr);

$lastrsv = lastrsv($signboard->nbr);

$filename = "Informations-panneau-DEV-$signboard->nbr.pdf";
...
<page backtop="5mm" backleft="10mm" backright="10mm" backbottom="5mm">
		...
		<tr>
			...
			<td class="box-75 text-uppercase">
                ...
                <h3 class="document-title">
                    ...
                    <div><?= "DEV-" . $signboard->nbr; ?></div>
                    <small class="text-grey"><?= $cmune->cmune_title ?></small>
    ...
    <table id="mytable">
        ...
        <tbody>
         ...
         <tr>
            ...
            <td class="box-25 text-uppercase ts-zx text-grey">Format</td>

            <td class="box-25 text-uppercase ts-zx text-grey text-center">Ville</td>

            <td class="box-25 text-uppercase ts-zx text-grey text-center">Zone</td>

            <td class="box-25 text-uppercase ts-zx text-grey text-right">Etat</td>

        </tr>

        <tr>

            <td class="box-25"><?= $size->size . "m²"; ?></td>

            <td class="box-25 text-center"><?= $ville->city_title; ?></td>

            <td class="box-25 text-center"><?= $zone->zone_title; ?></td>

            <td class="box-25 text-uppercase text-right"><?= $signboard->etat; ?></td>

        </tr>

        <tr><td colspan="4"><span class="text-uppercase text-grey">Situation géographique : </span><?= $signboard->geoloc; ?></td></tr>    
    ...
    <table id="mytable" class="mt-6 mb-3">
        ...
        <tr>
            ...
            <td class="box-50 bordertop text-uppercase ts-zx text-grey text-center"><?= count($lastcamps) > 1 ? "Campagnes" : "Campagne"; ?></td>

            <td class="box-50 bordertop text-uppercase ts-zx text-grey text-center"><?= count($lastrsv) > 1 ? "Réservations" : "Réservation"; ?></td>

        </tr>

        <tr>

            <td class="box-50 text-center ts-4x bolder"><?= count($lastcamps) ?></td>

            <td class="box-50 text-center ts-4x bolder"><?= count($lastrsv) ?></td>
    ...
    <table id="mytable" class="mt-6">
        ...
        <tr>
            ...
            <td><small class="header-title"><?= count($lastcamps) > 1 ? "Campagnes" : "Campagne"; ?> (<?= count($lastcamps); ?>)</small></td>
    ...
    <table class="table-stripped">
        ...
        <?php foreach($lastcamps as $item): ?>
            ...
            <tr>
                ...
                <td class="box-50 bg-striped">

                    <div><?= $item->camp_name; ?></div>
                    <small class="text-grey"><?= $item->e_name; ?></small>
                    
                </td>

                <td class="box-30 text-uppercase bg-striped">

                    <div><?= date('j/n/Y ',strtotime($item->debut)); ?></div>
                    <small class="text-grey"><?= date('j/n/Y ',strtotime($item->fin)); ?></small>

                </td>

                <td class="box-20 bg-striped"><?= $item->etat === "Closed" ? "Terminée" : $item->etat; ?></td>
            ...
            <tr>
                ...
                <td class="box-50 borderbottom bordertop">
                ...
                <td class="box-30 text-uppercase borderbottom bordertop">
                ...
                <td class="box-20 borderbottom bordertop"><?= $item->etat === "Closed" ? "Terminée" : $item->etat; ?></td>
        ...
        <tr>
            ...
            <td colspan="3" class="empty-table text-center">
                ...
                <span class="text-grey">Aucune campagnes récentes n'a été trouver pour ce panneau DEV-<?= $signboard->nbr; ?></span>
